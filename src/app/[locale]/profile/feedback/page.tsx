...
export default function FeedbackPage() {
  ...
  const [rating, setRatingState] = React.useState(0); // Renamed to avoid conflict with t.rating
  const [feedbackType, setFeedbackType] = React.useState("general");
  const [comments, setComments] = React.useState("");

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log("Feedback submitted:", { rating: rating, feedbackType, comments }); // Use state variable rating
    alert("Thank you for your feedback! (Simulated)");
    setRatingState(0); // Reset state variable
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-8">
            <div>
              ...
              <StarRatingInput rating={rating} setRating={setRatingState} />
            </div>

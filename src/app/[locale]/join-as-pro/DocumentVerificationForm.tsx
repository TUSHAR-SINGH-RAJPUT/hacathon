...
const createDocumentVerificationSchema = (t: any) => z.object({
  aadhaarNumber: z.string()
    .min(1, { message: t.aadhaarRequiredError || "Aadhaar number is required." })
    .refine(val => /^\d{4}\s?\d{4}\s?\d{4}$/.test(val) || /^\d{12}$/.test(val), {
      message: t.aadhaarInvalidError || "Enter a valid 12-digit Aadhaar number (e.g., XXXX XXXX XXXX or XXXXXXXXXXXX)."
  }),
  panNumber: z.string()
    .min(1, { message: t.panRequiredError || "PAN card number is required." })
    .refine(val => /^[A-Z]{5}[0-9]{4}[A-Z]{1}$/.test(val.toUpperCase()), {
      message: t.panInvalidError || "Enter a valid PAN card number (e.g., ABCDE1234F)."
  }),
  otherDocumentsDetails: z.string().max(500, { message: t.otherDocumentsTooLongError || "Details too long, max 500 characters."}).optional(),
});
...
export default function DocumentVerificationForm({ translations: t, locale }: DocumentVerificationFormProps) {
  ...
  function onSubmit(values: z.infer<typeof documentVerificationSchema>) {
    ...
    if (!initialData) {
      ...
      router.push(`/${locale}/join-as-pro`); 
      return;
    }

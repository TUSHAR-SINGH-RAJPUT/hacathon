...
import { useRouter } // Removed useParams as locale and bookingId come from props
  ...
  if (error || !bookingDetails) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-10rem)] animate-in fade-in duration-500">
        ...
        <p className="text-muted-foreground mb-6">{t.checkBookingIdContactSupport || "Please check ID or contact support."}</p>
  ...
  return (
    <div className="max-w-2xl mx-auto py-8 animate-in fade-in duration-500 space-y-6">
      <Button variant="outline" onClick={() => router.back()} className="mb-4">
        <ArrowLeft className="mr-2 h-4 w-4" /> {t.backButton || "Back"}
      </Button>
      ...
      <Card className="shadow-xl bg-card">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="aspect-video bg-muted rounded-lg flex items-center justify-center p-4 shadow">
            ...
            <p className="ml-4 text-muted-foreground text-center">{t.liveMapPlaceholder || "Live map tracking would appear here."}</p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 pt-4">
            <Button variant="outline" className="w-full text-primary border-primary hover:bg-primary hover:text-primary-foreground">
              <Phone className="mr-2 h-5 w-5" /> {t.callProvider ? t.callProvider(bookingDetails.provider.name.split(' ')[0]) : `Call ${bookingDetails.provider.name.split(' ')[0]}`}
            </Button>
            <Button variant="outline" className="w-full text-primary border-primary hover:bg-primary hover:text-primary-foreground">
              <MessageSquare className="mr-2 h-5 w-5" /> {t.messageProvider || "Message"}
            </Button>
          </div>
          <p className="text-xs text-center text-muted-foreground">{t.contactOptionsSimulated || "Contact options are simulated."}</p>
        </CardContent>
      </Card>
    </div>
  );

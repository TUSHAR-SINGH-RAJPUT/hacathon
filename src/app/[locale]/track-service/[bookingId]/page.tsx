...
export default function TrackServicePage() {
  ...
  useEffect(() => {
    const fetchDict = async () => {
      ...
      // Assuming translations for this page are under a key like "TrackServicePage"
      setT(dict.TrackServicePage || {
  ...
  }, [bookingId, t]); // Added t to dependency array
  ...
  if (error || !bookingDetails) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-10rem)] animate-in fade-in duration-500">
        ...
        <Button onClick={() => router.push(`/${locale}/`)}><ArrowLeft className="mr-2 h-4 w-4" /> {t.backToHome}</Button>
      </div>
    );
  }

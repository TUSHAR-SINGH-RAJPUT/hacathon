...
export default async function PlatformGuidePage({ params: { locale } }: GuidePageProps) {
  const dict = await getDictionary(locale);
  // Assuming translations for this page are flat in common.json or nested under a specific key
  // For simplicity, I'll assume Header might have some reusable appName or this page has its own section
  const t = dict.Header || {}; // Or dict.PlatformGuidePage if you create one
  const generalT = dict.LandingPage || {}; // For appName

  const createGuideSections = (currentLocale: string) => [ 
    { 
      title: t.navHome || "Homepage Overview", // Example fallback
      description: "Understand the main features accessible from our homepage.", 
      link: `/${currentLocale}/about/guide/homepage`, 
      icon: <Home className="h-6 w-6 text-primary" /> 
    },
    { 
      title: t.navPostJob || "Posting a New Job", 
    ...
    { 
      title: t.navBrowseServices || "Browsing Services & Provider Profiles", 
    ...
    { 
      title: t.jobList || "Using Your Job List (Cart) & Booking", 
    ...
    { 
      title: t.navJoinAsPro || "Joining as a Professional", 
    ...
    { 
      title: t.myAccount || "Managing Your Account", 
    ...
    <div className="animate-in fade-in duration-500 space-y-12">
      <section className="text-center py-12 md:py-16 bg-card rounded-xl shadow-lg">
        ...
        <h1 className="text-4xl md:text-5xl font-bold tracking-tight text-card-foreground">
          {generalT.appName || "kariGaar"} Platform Guide
        </h1>
        <p className="mt-4 text-lg text-muted-foreground max-w-2xl mx-auto">
          Welcome to your comprehensive guide for using {generalT.appName || "kariGaar"}! Whether you're a customer looking for services or a professional offering them, these guides will help you navigate our platform effectively.
        </p>
      </section>

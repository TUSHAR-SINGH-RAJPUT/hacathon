...
import { useRouter, useParams } from "next/navigation"; // Keep useParams if other dynamic params were used
...
export default function BookingConfirmationPageContent({ t, locale }: BookingConfirmationPageContentProps) {
  ...
  // const params = useParams(); // locale is now a prop
  // const locale = params.locale as string || 'en'; // locale is now a prop
  const { toast } = useToast();

  const [address, setAddress] = useState(customerAddress || "");
  const [paymentMethod, setPaymentMethod] = useState<"Cash on Delivery" | "Online Payment" | undefined>(undefined);
  const [isSubmitting, setIsSubmitting] = useState(false);
  // Removed: const [t, setT] = useState<any>({}); // t is now a prop

  // Removed useEffect for fetching dictionary

  useEffect(() => {
    if (Object.keys(t).length > 0 && cart.length === 0 && !isSubmitting) {
      toast({
  ...
  const handleConfirmBooking = () => {
    ...
    setTimeout(() => {
      const providerNames = cart.map(p => p.name).join(', ');
      toast({
        title: t.bookingConfirmedTitle,
        // Ensure t.bookingConfirmedDesc is a function or handle it appropriately
        description: typeof t.bookingConfirmedDesc === 'function' ? t.bookingConfirmedDesc(providerNames) : `Your request with ${providerNames} has been submitted. You will be contacted shortly.`,
        action: <Button onClick={() => router.push(`/${locale}/track-service/dummy-booking-123`)}>{t.trackService}</Button>
      });
      clearCart(); 

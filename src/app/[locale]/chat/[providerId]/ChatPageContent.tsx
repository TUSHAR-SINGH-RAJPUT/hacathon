...
import { useParams, useRouter } from 'next/navigation'; // useParams can get locale and providerId
    ...
    if (providerId) {
      ...
      if (foundProvider) {
        setMessages([
          {
            id: `msg-${Date.now()}`,
            text: t.initialMessage ? t.initialMessage(foundProvider.name) : `Hi there! I'm ${foundProvider.name}. How can I help you today?`,
            sender: 'provider',
            timestamp: new Date(),
          }
        ]);
  ...
  const handleSendMessage = () => {
    ...
    setTimeout(() => {
      const providerReply: ChatMessage = {
        ...
        text: t.simulatedReply ? t.simulatedReply(userMessage.text) : `Thanks for your message! (Simulated reply)`,
        sender: 'provider',
        timestamp: new Date(),
      };

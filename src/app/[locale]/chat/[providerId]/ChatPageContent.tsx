...
import { useRouter } from 'next/navigation'; // Removed useParams as providerId is now a prop
...
export default function ChatPageContent({ t, locale, providerId }: ChatPageContentProps) {
  ...
  useEffect(() => {
    if (providerId && Object.keys(t).length > 0) { 
      const foundProvider = dummyProviders.find(p => p.id === providerId);
      setProvider(foundProvider);
      if (foundProvider) {
        const initialMsgText = typeof t.initialMessage === 'function' ? t.initialMessage(foundProvider.name) : `Hi there! I'm ${foundProvider.name}. How can I help you today?`;
        setMessages([
          {
            id: `msg-${Date.now()}`,
            text: initialMsgText,
  ...
  const handleSendMessage = () => {
    if (newMessage.trim() === '' || !provider || Object.keys(t).length === 0) return;
    ...
    setTimeout(() => {
      const replyText = typeof t.simulatedReply === 'function' ? t.simulatedReply(userMessage.text) : `Thanks for your message! I'll get back to you shortly regarding "${userMessage.text.substring(0,20)}...". (This is a simulated reply)`;
  ...
  if (isLoadingProvider || Object.keys(t).length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-10rem)] animate-in fade-in duration-500">
        ...
        <p className="text-muted-foreground">{t.loadingChat || "Loading chat..."}</p>
      </div>
    );
  }
  ...
  if (!provider) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-10rem)] animate-in fade-in duration-500 text-center px-4">
        ...
        <h1 className="text-2xl font-semibold mb-2">{t.providerNotFound || "Provider Not Found"}</h1>
        <p className="text-muted-foreground mb-6">{t.couldNotInitiateChat || "Could not initiate chat."}</p>
        <Button onClick={() => router.push(`/${locale}/browse-providers`)} variant="outline" className="text-primary border-primary">
          <ArrowLeft className="mr-2 h-4 w-4" /> {t.backToProviders || "Back to Providers"}
        </Button>
      </div>
    );
  }
        ...
        <CardFooter className="p-4 border-t bg-card">
          ...
          >
            ...
            <Button type="submit" size="icon" className="bg-primary text-primary-foreground hover:bg-primary/90">
              ...
              <span className="sr-only">{t.sendMessage || "Send message"}</span>
            </Button>
          </form>
        </CardFooter>
      </Card>
      <p className="text-xs text-muted-foreground text-center mt-2">{t.simulatedChat || "This is a simulated chat."}</p>
    </div>
  );
}

...
const customerData = [
  ...
  { name: "New Users This Month", value: 300, fill: "hsl(var(--accent))" }, // Changed to accent for more visual diff
];

const jobsData = [
  { name: "Jobs Completed", value: 2800, fill: "hsl(var(--primary))" },
  { name: "Active Listings", value: 450, fill: "hsl(var(--accent))" }, // Changed to accent
];

const chartConfig = {
  value: { label: 'Count' },
  "Satisfied Customers": { label: "Satisfied Customers", color: "hsl(var(--primary))" },
  "New Users This Month": { label: "New Users This Month", color: "hsl(var(--accent))" },
    ...
    <div className="flex flex-col items-center space-y-12 md:space-y-16 animate-in fade-in duration-500">
      ...
      <section className="text-center py-12 md:py-20 bg-card rounded-xl shadow-lg w-full">
        <div className="container mx-auto px-4">
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight mb-6 text-transparent bg-clip-text bg-gradient-to-r from-primary via-accent to-foreground/80">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <Link href="/post-job" passHref>
              <Button size="lg" className="shadow-md transition-transform hover:scale-105">
      ...
      <section className="w-full py-12 md:py-16">
        ...
        <div className="grid md:grid-cols-3 gap-6 md:gap-8">
          {featureCards.map((feature) => (
            <Card key={feature.title} className="shadow-lg hover:shadow-xl transition-all duration-300 bg-background transform hover:-translate-y-1.5 hover:scale-[1.02]">
              <CardHeader className="items-center text-center">
                {React.cloneElement(feature.icon, { className: "h-10 w-10 text-primary group-hover:scale-110 transition-transform" })}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 md:gap-6">
          {serviceCategoriesData.map((service) => (
            <Link key={service.name} href={`/browse-providers?service=${encodeURIComponent(service.name)}`} passHref>
              <Card className={`group text-center p-4 shadow-md hover:shadow-xl transition-all duration-300 cursor-pointer bg-card hover:bg-secondary/70 aspect-square flex flex-col justify-center items-center transform hover:scale-105 hover:-translate-y-1`}>
                <div className={`mb-3 p-4 inline-block rounded-full ${service.bgColor} group-hover:scale-110 ${service.hoverBgColor} transition-all duration-300 ring-2 ring-transparent group-hover:ring-primary/50`}>
      ...
      {isClient ? (
        <section className="w-full py-12 md:py-16 bg-card rounded-xl shadow-xl">
          <h2 className="text-3xl font-bold text-center mb-10 md:mb-12 text-card-foreground">{dict.ourImpact}</h2>
          <div className="container mx-auto px-4 grid md:grid-cols-2 gap-8 items-center">
            <Card className="bg-background shadow-lg">
              <CardHeader className="items-center text-center">
                <PieChart className="h-10 w-10 text-primary" />
                <CardTitle className="text-xl text-foreground mt-2">{dict.customerGrowth}</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card className="bg-background shadow-lg">
              <CardHeader className="items-center text-center">
                ...
                <CardTitle className="text-xl text-foreground mt-2">{dict.jobConnections}</CardTitle>
              </CardHeader>
              <CardContent>
                <ChartContainer config={chartConfig} className="mx-auto aspect-square max-h-[250px]">
                   <RechartsResponsiveContainer width="100%" height="100%">
        ...
        <section className="w-full py-12 md:py-16 bg-card rounded-xl shadow-xl">
           <h2 className="text-3xl font-bold text-center mb-10 md:mb-12 text-card-foreground">{dict.ourImpact}</h2>
           <div className="container mx-auto px-4 grid md:grid-cols-2 gap-8 items-center">
            <div className="bg-background rounded-lg shadow p-6 aspect-square flex items-center justify-center text-muted-foreground min-h-[300px]">Loading chart...</div>
            <div className="bg-background rounded-lg shadow p-6 aspect-square flex items-center justify-center text-muted-foreground min-h-[300px]">Loading chart...</div>
           </div>
           <p className="text-center text-muted-foreground mt-8 text-sm">(Numbers are illustrative)</p>
        </section>
      ...
      <section className="w-full py-12 md:py-16 text-center bg-secondary rounded-xl shadow-lg">
         <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold mb-6 text-secondary-foreground">{dict.joinThousands}</h2>
